import { pgTable, uniqueIndex, bigserial, varchar, timestamp, index, foreignKey, bigint, integer, uuid, boolean, text, primaryKey } from "drizzle-orm/pg-core"
...
export const session = pgTable("session", {
	...
	userId: uuid("user_id").default(sql`uuid_generate_v4()`).notNull(),
});
...
(table) => {
	return {
		indexVerificationTokenOnIdentifierAndToken: uniqueIndex("index_verification_token_on_identifier_and_token").using("btree", table.identifier.asc().nullsLast(), table.token.asc().nullsLast()),
	}
});
...
(table) => {
	return {
		indexAuthenticatorsOnCredentialId: uniqueIndex("index_authenticators_on_credentialID").using("btree", table.credentialId.asc().nullsLast()),
