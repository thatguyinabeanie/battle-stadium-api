...
export const matches = pgTable(
  ...
  (table) => {
    return {
      ...
      indexMatchesOnRoundAndPlayersUnique: uniqueIndex("index_matches_on_round_and_players_unique").using(
        "btree",
...
export const organizationStaffMembers = pgTable(
  ...
  (table) => {
    return {
      indexOrganizationStaffMembersOnOrganizationId: index("index_organization_staff_members_on_organization_id").using(
        "btree",
...
export const phasePlayers = pgTable(
  ...
  (table) => {
    return {
      indexPhasePlayersOnPlayerId: index("index_phase_players_on_player_id").using(
        "btree",
        table.playerId.asc().nullsLast(),
      ),
      indexTournamentPhasePlayersOnPhase: index("index_tournament_phase_players_on_phase").using(
        "btree",
...
export const tournamentFormats = pgTable(
  ...
  (table) => {
    return {
      indexTournamentFormatsOnFormatId: index("index_tournament_formats_on_format_id").using(
        "btree",
        table.formatId.asc().nullsLast(),
      ),
      indexTournamentFormatsOnTournamentId: index("index_tournament_formats_on_tournament_id").using(
        "btree",
        table.tournamentId.asc().nullsLast(),
      ),
...
export const players = pgTable(
  ...
  (table) => {
    return {
      indexPlayersOnPokemonTeamId: index("index_players_on_pokemon_team_id").using(
        "btree",
...
export const rounds = pgTable(
  ...
  (table) => {
    return {
      ...
      indexRoundsOnPhaseIdAndRoundNumber: uniqueIndex("index_rounds_on_phase_id_and_round_number").using(
        "btree",
        table.phaseId.asc().nullsLast(),
...
export const pokemon = pgTable(
  ...
  (table) => {
    return {
      indexPokemonOnPokemonTeamId: index("index_pokemon_on_pokemon_team_id").using(
        "btree",
        table.pokemonTeamId.asc().nullsLast(),
      ),
...
export const session = pgTable(
  ...
  (table) => {
    return {
      indexSessionOnUserId: index("index_session_on_userId").using("btree", table.userId.asc().nullsLast()),
    };
  },
);
...
export const verificationToken = pgTable(
  ...
  (table) => {
    return {
      indexVerificationTokenOnIdentifierAndToken: uniqueIndex("index_verification_token_on_identifier_and_token").using(
        "btree",
...
export const users = pgTable(
  "users",
  {
    ...
    name: varchar("name"),
    emailVerified: timestamp("emailVerified", { precision: 6, mode: "date" }),
    image: text("image"),
  },
  (table) => {
    return {
      indexUsersOnConfirmationToken: uniqueIndex("index_users_on_confirmation_token").using(
        "btree",
      ...
      indexUsersOnResetPasswordToken: uniqueIndex("index_users_on_reset_password_token").using(
        "btree",
        table.resetPasswordToken.asc().nullsLast(),
      ),
      indexUsersOnUnlockToken: uniqueIndex("index_users_on_unlock_token").using(
        "btree",
        table.unlockToken.asc().nullsLast(),
      ),
...
export const formats = pgTable(
  ...
  (table) => {
    return {
      ...
      indexFormatsOnNameAndGameId: uniqueIndex("index_formats_on_name_and_game_id").using(
        "btree",
...
export const tournaments = pgTable(
  ...
  (table) => {
    return {
      ...
      indexTournamentsOnNameAndOrganizationId: uniqueIndex("index_tournaments_on_name_and_organization_id").using(
        "btree",
        table.name.asc().nullsLast(),
        table.organizationId.asc().nullsLast(),
      ),
      indexTournamentsOnOrgIdNameStartDate: uniqueIndex("index_tournaments_on_org_id_name_start_date").using(
        "btree",
        table.organizationId.asc().nullsLast(),
        table.name.asc().nullsLast(),
        table.startAt.asc().nullsLast(),
      ),
      indexTournamentsOnOrganizationId: index("index_tournaments_on_organization_id").using(
        "btree",
        table.organizationId.asc().nullsLast(),
      ),
