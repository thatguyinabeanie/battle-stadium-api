...
const generateMockUserData = (count: number): Users[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const user: Users = {
      ...
      country: selectedCountry ?? { name: "Unknown", icon: <></> },
      role: selectedRole || "Unknown",
      workerType: Math.random() > 0.5 ? "Contractor" : "Employee",
      status:
        Math.random() > 0.5 ? "Active" : Math.random() > 0.5 ? "Paused" : Math.random() > 0.5 ? "Vacation" : "Inactive",
      startDate: new Date(new Date().getTime() - Math.random() * (24 * 60 * 60 * 1000 * 40)),
      teams: ["Design", "Product", "Marketing", "Management", "Engineering", "Sales", "Support", "Other"].filter(
        () => Math.random() > 0.5,
      ),
    };
