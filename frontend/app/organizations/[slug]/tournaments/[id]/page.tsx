...
export async function generateStaticParams() {
  ...
  return tournaments.map(({ organization, id }) => ({ slug: organization.slug, id: id.toString() }));
}

export default async function OrganizationTournament({ params }: Readonly<{ params: { slug: string; id: string } }>) {
  const tournament = (await getTournament(parseInt(params.id))).data;
